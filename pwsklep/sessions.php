<?php
	...
	class session
	{
		...
		public function __construct()
		{
			...
			$stmt = $pdo->prepare('SELECT session_id, updated_at, salt_token, user_id, uniq_info, browser, ip FROM sessions WHERE session_id = :sid AND uniq_info = :info AND updated_at > :updated AND browser = :browser AND ip = :ip');
		...
		function updateSession(user $user)
		{
			...
			$stmt->bindValue(':time', time(), PDO::PARAM_INT);
			$stmt->bindValue(':newId', $newId, PDO::PARAM_STR);//znacznik
